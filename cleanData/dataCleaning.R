...
library(rvest)
library(dplyr)
#setwd("C:\\Users\\vignesh.venugopal\\Documents\\GitHub\\socialMediaMarketing\\data")
GalaxyS8_tweets <- read_csv("#TouchOfCare_tweets.csv")
GalaxyS8_youtube <- read_csv("#TouchOfCare_youtube.csv")
GalaxyS8_facebook_statuses <- read_csv("#TouchOfCare_facebook_statuses.csv")
...
Cleaned_Corpus_fb<-tm_map(Cleaned_Corpus_fb,removeWords,c(stopwords("english"),"http","https"))
...
data_samsun<-data_samsun%>%filter(freq > 1)
word_clound_Samsung<-wordcloud(words = data_samsun$word,freq = data_samsun$freq,min.freq = 3,max.words = 200,random.order = FALSE,rot.per = 0.35,colors = brewer.pal(8,"Dark2"))
