plotWordMetrics <- function(corpus, wordList, wordFrequency, correlationValue, associationWord, associationValue){
    ...
    wordCorr <- apply_as_df(myCorpus, word_cor, word = "emotional", r = 0.05)
    ...
    wordcloud(words = names(word.freq), freq = word.freq, min.freq = 7, random.order = F, colors = brewer.pal(8, "Dark2"), max.words =100)
    plot(wordCorr)
